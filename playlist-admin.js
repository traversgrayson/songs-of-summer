...
function isYouTubeLink(link) {
  return !!link && /youtu\.?be/.test(link);
}
...
async function generatePlaylist() {
  ...
  try {
    ...
    resultEl.innerHTML = `
      ...
      ${skippedYoutube ? `<p class="result-line">${skippedYoutube} skipped (YouTube-only links)</p>` : ''}
      <p class="result-line">Playlist ID (paste into <code>playlist.js</code> on the public page):</p>
      <p class="result-id">${playlistId}</p>
